...
string appendAndDelete(string s, string t, int k) {
    ...
    // Find the common length of the prefix between s and t
    int common_length = 0;
    while (common_length < s_length && common_length < t_length && s[common_length] == t[common_length]) 
    {
        common_length++;
    }
    
    // Calculate the number of operations needed
    int total_operations = s_length + t_length - 2 * common_length;
    
    // Check if it's possible to convert s to t within k operations
    if (k>=t_length+s_length || k>=total_operations && (k-total_operations)%2==0)
    {
        return "Yes";
    }
     else 
